  ...
  return [
    ...
    params.availableTools.has("message")
      ? [
          ...
          "- Browser screenshots and other tool-generated media are not auto-attached to external chat replies. If you want to send them, use `message` with `media`, `path`, or `filePath`.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    js_repl:
      "Run persistent JavaScript with top-level await; use openclaw.tool(...), openclaw.emitImage(...), and openclaw.browser.connect() for interactive workflows",
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- js_repl: run persistent JavaScript with OpenClaw helpers for nested tool calls and interactive browser work",
    ...
    "",
    "## Tool Call Style",
    "For a single routine, low-risk tool call, you may call the tool directly without extra narration.",
    ...
    "Never write tool calls as plain text, pseudo-markup, or raw JSON blobs in normal assistant text.",
    ...
    "For browser work, attach once with `browser` action=`session_attach`, then prefer action=`inspect` for broad page context. Use snapshot `mode=efficient` only when you explicitly want a compact snapshot.",
    "If a tool result indicates output truncation, acknowledge it and run a narrower follow-up command before concluding.",
